...
public class AudioManager : MonoBehaviour
{
    ...
    public void Play(SoundEnum soundEnum, float fadeTime = 0, bool loops = false, bool ignoreListenerPause = false)
    {
        StartAudioAction(AudioAction.Start, _soundEnumDictionary[soundEnum], fadeTime, loops, ignoreListenerPause);
    }

    public void Play(SoundEnum soundEnum, AudioSource audioSource, float fadeTime = 0, bool loops = false, bool ignoreListenerPause = false)
    {
        StartAudioAction(AudioAction.Start, _soundEnumDictionary[soundEnum], fadeTime, loops, ignoreListenerPause, audioSource);
    }
    ...
    {
        StartAudioAction(AudioAction.Stop, null, fadeTime, false, false, audioSource);
    }
    ...
    private void StartAudioAction(AudioAction action, Sound sound, float fadeTime, bool loops, bool ignoreListenerPause = false, AudioSource audioSource = null)
    {

        if (action is AudioAction.Restart)
        {
            StartAudioAction(AudioAction.Stop, sound, fadeTime, loops, ignoreListenerPause);
            StartAudioAction(AudioAction.Start, sound, fadeTime, loops, ignoreListenerPause);
            return;
        }
        ...
        if (action is AudioAction.Start)
        {
            ...
            {
                Sound = sound,
                Routine = StartCoroutine(AudioActionCoroutine(action, audioSource, sound, soundClip, fadeTime, loops, ignoreListenerPause)),
                Clip = soundClip
            };
        }
        if (action is AudioAction.Stop)
        {
            ...
            foreach (var source in audioSources)
            {
                ...
                _jobDictionary[source].Routine = StartCoroutine(AudioActionCoroutine(action, source, sound, null, fadeTime, loops, ignoreListenerPause));
            }
        }
    }

    private IEnumerator AudioActionCoroutine(AudioAction action, AudioSource source, Sound sound, Sound.Clip clip, float fadeTime, bool loops, bool ignoreListenerPause)
    {
        ...
        source.volume = (action == AudioAction.Stop) ? 0 : initialVolume;
